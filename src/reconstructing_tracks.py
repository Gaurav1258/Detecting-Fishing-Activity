...
from pipeline_noLatLon import keep_columns, X_y_split, columns_models_dict
...
email_sender = "g.knallhart@gmail.com"
email_password = "eeknbppjzhbhuypb"
email_receiver = "gauravjshrivastava10@gmail.com"

#set the subject of the email: 
subject = 'Overfishing Detected'
...
def make_prediction(X, pickeled_model):
    print(pickeled_model)
    model = np.load(pickeled_model, allow_pickle=True)
    # file = open(pickeled_model, 'rb')
...
def plot_track_predictions(df, mod_num, mmsi, pickeled_model, up, down, left, right):
    ...
    fig_name = '../results/best_reconstructions/' + \
        str(mmsi) + '_transparency_' + \
        pickeled_model[len('../results/final_models/'):-len('.pkl')]+'.png'
    ...
    if(len(x1)> 100):
        print("Overfishing detected by mmsi: ", best_mmsi)
        body = str(best_mmsi) + ' :overfishing detected'
        em.set_content(body)
